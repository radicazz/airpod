...
- Provide simple, predictable lifecycle commands (`start`, `stop`, `status`, `logs`, `doctor`, `config`, and `clean`) with `start --init` covering bootstrap flows. These commands must:
