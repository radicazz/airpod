...
def pull_hf_cmd(
    repo: str = typer.Argument(..., help="HuggingFace repo ID (e.g., bartowski/Llama-3.2-3B-Instruct-GGUF)"),
    file: Optional[str] = typer.Option(None, "--file", "-f", help="GGUF filename to download"),
    name: Optional[str] = typer.Option(None, "--name", "-n", help="Model name in Ollama"),
    help_: bool = command_help_option(),
) -> None:
    ...
    try:
            ...
            if len(gguf_files) == 1:
                ...
                while True:
                    ...
                    if 1 <= selection <= len(gguf_files):
                        ...
                        break
                    console.print(f"[error]Invalid selection. Please choose 1-{len(gguf_files)}[/]")
        ...
        with Progress(
            ...
            import_task = progress.add_task("Importing to Ollama", total=100, visible=False)
